...
def extract_data_from_pdf(pdf_data):
    with BytesIO(pdf_data) as pdf_buffer:
                ...
                if p0 == pdf.pages[0]:
                    for table in p0_tables:
                        ...
                        table = [
                            ...
                            if any(item not in (None, "") for item in sublist)
                        ]
                        for i in table:
                    ...
                    for table in p0_tables:
                        for index, i in enumerate(table):
                            ...
                            if i[0] == "":
                                ...
                                if len(i) > 1:
                                    processed_dictionary[last_key] = (
                                        processed_dictionary[last_key] + i[1]
                                    )
        ...
        if "Contact Person" in processed_dictionary.keys():
            processed_dictionary["Contact person"] = processed_dictionary[
                "Contact Person"
            ]
